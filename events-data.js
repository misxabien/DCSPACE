...
(function (global) {
  ...
  function getEventDetailUrl(id, context) {
    if (context === 'explore') {
      return './20-event-explore.html?id=' + encodeURIComponent(id);
    }
    if (context === 'attendance') {
      return './25-attendance-details.html?id=' + encodeURIComponent(id);
    }
    return './19-event-details.html?id=' + encodeURIComponent(id);
  }

  function getEventSubmitUrl(id) {
    return './21-event-submit.html?id=' + encodeURIComponent(id);
  }
